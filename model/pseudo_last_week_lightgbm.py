...
def pseudo_last_week_lightgbm(x_train,x_test, fake, feature):
    ...
    X_train = pd.concat([X_train, fake[feature]]).reset_index(drop = True)
    y_train = pd.concat([y_train, fake['click_mode']]).reset_index(drop = True)
