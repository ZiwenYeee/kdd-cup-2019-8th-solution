...
def linear_split(X, feature):
    ...
    X_train = X.loc[X['req_time'] < '2018-11-14', feature].reset_index(drop = True)

    X_test = X.loc[(X['req_time'] > '2018-11-14') &
    ...
    y_train = X.loc[X['req_time'] < '2018-11-14', 'click_mode'].reset_index(drop = True)

    y_test = X.loc[(X['req_time'] > '2018-11-14') &
...
def last_week_lightgbm(x_train,x_test, feature):
    ...
    X_train, X_test, X_valid, y_train, y_test, y_valid = linear_split(x_train, feature)
    ...
    lgb_prediction = clf.predict(X_valid)
    print('one week for validation strategy,last week F1 : %.6f' %(f1_score(y_valid,
    lgb_prediction.argmax(axis = 1), average = 'weighted') ) )
...
#         )
    ...
    oof_all = np.vstack((oof_train, oof_valid))
    oof_test_2 = lgb_test
    ...
    print('one week for validation strategy,last week F1 : %.6f' %(f1_score(y_valid,
    oof_valid.argmax(axis = 1), average = 'weighted') ) )
    del clf, dtrain, dtest
    gc.collect()
    return oof_all, oof_test_2, feature_importance_df
